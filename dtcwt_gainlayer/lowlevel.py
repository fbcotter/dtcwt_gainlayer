...
class GainLayer_f(torch.autograd.Function):
    ...
    def forward(ctx, x, lp, bpr, bpi, h0o, h1o):
        ...
        pad_i = (0, 0, 0, 0, 0, 0)
        for l in range(6):
            pad_r[l] = bpr[l].shape[0] // 2
            pad_i[l] = bpi[l].shape[0] // 2
        ...
        reals = [deg15r, deg45r, deg75r, deg105r, deg135r, deg165r]
        imags = [deg15i, deg45i, deg75i, deg105i, deg135i, deg165i]

        # real output = r*r - i*i
        outs_r = [None,] * 6
        outs_i = [None,] * 6
        for l in range(6):
            outs_r[l] = F.conv2d(reals[l], bpr[l], padding=pad_r[l]) - \
                F.conv2d(imags[l], bpi[l], padding=pad_i[l])
            outs_i[l] = F.conv2d(reals[l], bpi[l], padding=pad_
