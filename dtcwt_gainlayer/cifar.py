...
import tarfile
import pickle
from dtcwt_gainlayer.utils import download, md5, convert_to_one_hot
...
def get_data(basedir, in_size, dataset='cifar10', batch_size=128,
             trainsize=50000, seed=random.randint(0, 10000), perturb=True):
    """ Provides a pytorch loader to load in cifar10/100

    Args:
        basedir (str): path to directory where the cifar10/100 data lives
        in_size (int): input size in pixels
        dataset (str): cifar10 or cifar100
        batch_size (int): batch size for training set. currently uses batch 100
            always for val set
        trainsize (int): number of images to use for the training set
    ...
    if dataset == 'cifar10':
        ...
        trainset = torchvision.datasets.CIFAR10(
            root=basedir, train=True, download=True,
            transform=transform_train)
        idxs = subsample(True, trainsize)
        trainset = torch.utils.data.Subset(trainset, idxs)
        testset = torchvision.datasets.CIFAR10(
            root=basedir, train=False, download=False,
